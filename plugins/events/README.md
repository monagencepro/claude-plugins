...
Pour (re)lier une machine à la demande (forme namespacée car `/login` est une
commande native de Claude Code) :
...
- Liaison OAuth : aucun secret n'est embarqué dans le plugin. L'autorisation mint un token côté plateforme, rapatrié **une seule fois** et stocké localement dans `~/.claude/hooks/.monagence-token`.
...
|---|---|---|
| `MONAGENCE_TOKEN` | _(vide)_ | Court-circuite la liaison OAuth avec un token explicite. Sinon lu depuis `~/.claude/hooks/.monagence-token`. |
| `MONAGENCE_BASE_URL` | `https://app.monagence.pro` | Base de la plateforme (device flow + ingestion). |
| `MONAGENCE_ENDPOINT` | `$MONAGENCE_BASE_URL/api/claude/events` | Endpoint d'ingestion (override fin). |

## Confidentialité / révocation

Le token reste local. Pour couper l'envoi : désactive le plugin
(`/plugin`), supprime `~/.claude/hooks/.monagence-token`, ou révoque le token
...
`~/.claude/hooks/monagence-events.log`.
