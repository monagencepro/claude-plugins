...
Lance le device-flow login du plugin maw-events pour (re)lier cette machine.
...
```
node "${CLAUDE_PLUGIN_ROOT}/scripts/maw-login.mjs"
```
...
se trouve dans le dossier `scripts/` du plugin `maw-events` installé.
